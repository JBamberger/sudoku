...
class DigitClassifier::Impl
{
  ...
  public:
    Impl()
    {
        hogDescriptor = HOGDescriptor(Size(20, 20),                                // winSize
                                      Size(8, 8),                                  // blocksize
  ...
  private:
    ...
    /**
     * This function deskews an input image of size 20x20 grayscale and then
    ...
    ////std::cout << output.size().width << "x" << output.size().height <<
    /// std::endl;
    // return output;
...
int
DigitClassifier::classify(const cv::Mat& img)
{
