...
static bool is_valid_value(std::vector<std::vector<int>> board, size_t row, size_t col, int value) {
...
static void solveImpl(std::vector<std::vector<int>> board, size_t pos) {
	...
	if (pos == S * S) {
		...
		if (board[row][col] > 0) {
			solveImpl(board, pos + 1); // the field is already occupied, go to next one
		}
		else {
			for (int i = 1; i <= 9; i++) {
		...
		throw std::exception("There exists no solution for this Sudoku."); // there is no solution to the sudoku
	}
}
