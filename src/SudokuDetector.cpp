 ...
 */
void
preprocess_image(Sudoku& sudoku, cv::Mat& inverted)
{
 ...
 */
cv::Rect
find_rough_crop_region(const cv::Mat& inverted)
{
 ...
 */
std::vector<cv::Vec2f>
nms_line(std::vector<cv::Vec3f>& lines, const float min_dist = 20.f)
{
 ...
 * vertical axis.
 */
std::pair<std::vector<cv::Vec2f>, std::vector<cv::Vec2f>>
find_lines(const cv::Mat& image, double theta)
{
 ...
 */
cv::Point2f
intersect(const cv::Vec2f& l1, const cv::Vec2f& l2, const double epsilon = 1e-15)
{
 ...
 */
std::pair<cv::Vec2f, cv::Vec2f>
find_min_max_line_pair(std::vector<cv::Vec2f> lines, float angleFunction(float))
{
 ...
 */
void
detect_sudoku_corners(Sudoku& sudoku, const cv::Mat& inverted)
{
 ...
 */
void
compute_unwarp_transform(Sudoku& sudoku)
{
...
cv::Mat
keep_largest_blob(const cv::Mat& in)
{
    int count = 0;
    int max = -1;
    cv::Point2i maxPt;
...
void
get_cell_contents(Sudoku& sudoku)
{
    cv::Mat img;
    cv::Mat threshed;
