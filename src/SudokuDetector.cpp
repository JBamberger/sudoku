 ...
 */
void preprocess_image(Sudoku& sudoku, cv::Mat& inverted) {
 ...
 */
cv::Rect find_rough_crop_region(const cv::Mat& inverted) {
...
/**
 * This function performs non-maximum suppression for lines by eliminating all lines
 * which are closer than min_dist.
 */
...
/**
 * Function computes a set of horizontal and vertical lines in the given image. The threshold theta
 * specifies the allowed deviation from the horizontal / vertical axis.
 */
...
/**
 * Function which computes the intersection of two lines given as angle and distance from origin.
 */
...
/**
 * This function computes the minimal and maximal lines in horizontal or vertical direction.
 * The angleFunction is used to compute the projected distance of the line.
 */
std::pair<cv::Vec2f, cv::Vec2f> find_min_max_line_pair(std::vector<cv::Vec2f> lines, float angleFunction(float)) {
...
void detect_sudoku_corners(Sudoku& sudoku, const cv::Mat& inverted) {
	...
	const cv::Point2f delta{
		static_cast<float>(sudoku.bbox.width) / static_cast<float>(dsize.width),
	...
	sudoku.corners = Quad{
		uncrop_coords(intersect(min_hline, min_vline)),
...
/**
 * This function computes the aligned sudoku image and the transformations from and to transformed space.
 */
...
/**
 * This is the interface function which performs all steps of the sudoku detection.
 */
